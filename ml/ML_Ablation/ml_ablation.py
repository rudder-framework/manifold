...
Usage:
    ...
    python -m prism.entry_points.ml_ablation --target RUL --show-discovery
...
def main():
    ...
    # Handle target as column OR as signal_id (long format)
    if args.target in observations.columns:
        # Target is a column - use max (initial RUL for run-to-failure)
        target = observations.group_by(entity_col).agg(
            pl.col(args.target).max().alias('target')
        ).sort(entity_col)
    elif 'signal_id' in observations.columns:
        # Target might be stored as a signal_id (long format schema)
        target_signal = args.target
        target_rows = observations.filter(pl.col('signal_id') == target_signal)
        if len(target_rows) == 0:
            print(f"Error: Target '{args.target}' not found as column or signal_id")
            sys.exit(1)
        # Use MAX RUL (initial remaining life) not last (which is 0 for run-to-failure)
        target = target_rows.group_by(entity_col).agg(
            pl.col('value').max().alias('target')
        ).sort(entity_col)
        print(f"Target range: {target['target'].min():.0f} → {target['target'].max():.0f}")
        # Remove target from observations for feature extraction
        observations = observations.filter(pl.col('signal_id') != target_signal)
    else:
    ...
    if vector_path.exists():
        ...
        if entity_col in vector.columns:
            ...
            results.append(result)
            print(f"  Features: {result['n_features']}")
            print(f"  RMSE: {result['rmse']:.2f} (Δ {delta:+.2f})")
    ...
    if len(results) >= 2:
        ...
        if biggest_stage:
            print(f"Biggest contributor: {biggest_stage} (Δ -{biggest_delta:.2f} RMSE)")
